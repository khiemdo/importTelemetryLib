...
class SerialController(object):
    ...
    def PersistentStorageMsgHandle(self,topic,data):
        unpacked_data = self.cStructWorker.unpack(data)
        self.logger.info(unpacked_data)
        root = self.model.rootItem.node()
        self.__AssignValuesIntoParentNode(root,unpacked_data,0)

    def __AssignValuesIntoParentNode(self, node, data,index = 0):
        nodeLength = len(node)
        if (nodeLength==0):
            value = data[index]
            index+=1
